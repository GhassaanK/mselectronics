...
import { Trash2, Plus, GripVertical, Eye, EyeOff, Pencil, X, Check } from "lucide-react"
...
export function BannerForm({ initialBanners }: Props) {
  ...
  function updateEditForm(key: keyof EditState, value: string) {
    ...
    <div className="space-y-4">
      ...
      {banners.map((banner, i) => (
        <Card key={banner.id} className="p-3">
          ...
          {editingId === banner.id && editForm ? (
            /* ── Edit mode ─────────────────────────────── */
            <div className="space-y-3">
              <p className="text-sm font-semibold">Edit Banner #{i + 1}</p>
              ...
              <div className="rounded-md border border-dashed p-3 text-sm text-muted">
                {editForm.imageUrl ? (
                  <div className="flex items-center gap-2">
                    ...
                    <img src={editForm.imageUrl} alt="" className="h-12 w-20 rounded object-cover" />
                    <button
                      onClick={() => { updateEditForm("imageUrl", ""); updateEditForm("imagePublicId", "") }}
                  ...
                  <CldUploadWidget
                    ...
                    onSuccess={(result) => {
                      ...
                      updateEditForm("imageUrl", info.secure_url)
                      updateEditForm("imagePublicId", info.public_id ?? "")
                    }}
                  >
                    {({ open }) => (
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <Input
                  ...
                  onChange={(e) => updateEditForm("headline", e.target.value)}
                />
                <Input
                  placeholder="Subheadline"
                  value={editForm.subheadline}
                  onChange={(e) => updateEditForm("subheadline", e.target.value)}
                />
                <Input
                  placeholder="CTA label (e.g. Shop Now)"
                  value={editForm.ctaLabel}
                  onChange={(e) => updateEditForm("ctaLabel", e.target.value)}
                />
                <Input
                  placeholder="CTA link (e.g. /shop)"
                  value={editForm.ctaHref}
                  onChange={(e) => updateEditForm("ctaHref", e.target.value)}
                />
              </div>

              <div className="flex items-center gap-3">
                <Button type="button" size="sm" onClick={() => handleSaveEdit(banner.id)}>
      ...
      {adding ? (
        ...
        <Button
          ...
          variant="outline"
          size="sm"
          onClick={() => { setAdding(true); setEditingId(null); setEditForm(null) }}
          className="gap-1.5"
        >
          <Plus size={14} />
          Add Banner
